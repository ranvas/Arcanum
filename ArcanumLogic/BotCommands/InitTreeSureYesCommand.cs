...
{
    public class InitTreeSureYesCommand:RootSendCommand
    {
        public override string Command { get; set; } = $"/{DispatcherCommandsEnum.init_tree_sure_yes}";
        public override Task<string> ExecuteAsync(string? param, Update executionContext, IBotAdapter service)
        {
            var message = DispatcherCommandsEnum.init_tree.ToString();
