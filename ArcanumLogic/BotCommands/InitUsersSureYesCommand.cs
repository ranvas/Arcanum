...
{
    public class InitUsersSureYesCommand : RootSendCommand
    {
        public override string Command { get; set; } = $"/{DispatcherCommandsEnum.init_users_sure_yes}";

        public override Task<string> ExecuteAsync(string? param, Update executionContext, IBotAdapter service)
        {
            var message = DispatcherCommandsEnum.init_users.ToString();
