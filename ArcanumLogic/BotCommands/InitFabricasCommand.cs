...
{
    public class InitFabricasCommand : RootSendCommand
    {
        public override string Command { get; set; } = $"/{DispatcherCommandsEnum.init_fabricas}";
        public override Task<string> ExecuteAsync(string? param, Update executionContext, IBotAdapter service)
        {
            var message = DispatcherCommandsEnum.init_fabricas.ToString();
